...
CHROMA_PATH = "chroma_db_pydantic_ai_4"
from graph import GenderGraphHandler
NEO4J_URI = "bolt://localhost:7689"
NEO4J_USER = "neo4j"
NEO4J_PASSWORD = "test12345"

from agents import extractor_agent, query_generator_agent
from base import write_result2, CharactersList, FusionQuestionOut
...
@retry(stop=stop_after_attempt(15), wait=wait_fixed(60))
async def generate_query_rank(text) -> FusionQuestionOut:
    try:
        result = await query_generator_agent.run(f"Текст для анализа: {text}")
...
def reciprocal_rank_fusion(results_lists: List[List[str]], k: int = 60) -> List[str]:
    """Алгоритм RRF для объединения результатов разных поисковых запросов."""
    fused_scores = {}
...
async def fetch_context_for_chunk(chunk_text: str, book_name: str, n: int = 3) -> str:
    """Извлекает примеры перевода, используя Fusion Search."""
    try:
        # Генерируем вариации запросов
        response = await generate_query_rank(chunk_text)
        queries = [response.first_query, response.second_query, response.third_qury]
        ...
        for q in queries:
            ...
            if res['documents'] and res['documents'][0]:
                ...
                results_lists.append(formatted_results)

        if not results_lists:
            return "Примеры стиля: (база памяти для этой книги пока пуста)\n"

        fused = reciprocal_rank_fusion(results_lists)
        ...
        for chapter_name, pairs in chapters.items():
            ...
            if i >= 32 or chapter_find:
                ...
                try:
                    ...
                    write_result2(res,4,i,book_name)
